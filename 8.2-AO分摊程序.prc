create or replace procedure p_abc_fct_ao_dist(p_to_dt date default sysdate) is
  /*************************************************************
   author  : blt
   created : 2019-06-30
   purpose :
...
begin
  v_sqlstate  := '变量赋值';
  v_proc_name := 'p_abc_fct_ao_dist';
  v_fm_date   := trunc(add_months(p_to_dt, -1), 'MM');
  v_to_date   := trunc(p_to_dt, 'MM');
  v_month     := to_char(v_fm_date, 'YYYYMM');

  v_sqlstate := '删除数据';
  delete abc_fct_ao_dist_tmp01;
  delete abc_fct_ao_dist_tmp02;
  delete abc_fct_ao_dist_tmp03;
  delete abc_fct_ao_dist_tmp04;
  delete abc_fct_ao_dist a where a.month_code = v_month;
  ...
  insert into abc_fct_ao_dist_tmp04
    select a.mode_code,
      ...
      left join abc_fct_ao_driv b
       ...
       and nvl(a.fm_car, 'abc') = nvl(b.car_no, 'abc')
