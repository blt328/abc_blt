...
begin
  ...
  for v_waybill in (select * from abc_bsl_waybill) loop
    v_sqlstate := '生成比率';
    select dbms_random.value(0, 5) / 3,
           dbms_random.value(0, 5) / 3,
           dbms_random.value(0, 5) / 2,
           dbms_random.value(0, 3),
           dbms_random.value(0, 5) / 2,
           dbms_random.value(0, 3)
      into v_pep, v_mng, v_div, v_mate, v_car, v_fot
      from dual;
      ...
      if v_sub.reso_name = '运输费' then
        ...
        values
          (v_yyyymm,
           ...
           null,
           v_waybill.rec_city || '-' ||
           lpad(round(dbms_random.value(0, 5), 0), 5, '0'),
